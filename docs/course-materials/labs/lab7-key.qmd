...
| Column | Meaning |
|----|----|
| `pwsid` | ID of the public water system |
| `pwsname` | Name of the public water system |
| `pctpov` | Percent of the district under the poverty line |
| `pcthisp` | Percent of the district that is Hispanic |
| `health` | Count of SDWA violations |
| `medianincomehouse` | Median household income in the district |
| `violation` | Whether or not the district had a SDWA violation (binary) |
| `pcthisp_bin` | `pcthisp`, binned to 10% intervals |
| `pctpov_bin` | `pctpov`, binned to 10% intervals |
...
```{r}
summary(class_model)$coefficients["pctpov", 4] < 0.05
```
...
confint(ej_model) # 0.00019 - 0.00046
